...
判停延迟 silence_duration_ms 通过环境变量 VAD_SILENCE_MS 配置（默认 300ms，
实测 800ms 时转向延迟 ~1.3s，300ms 目标 ~0.8s；低于 300 容易抢话）。
"""
...
class QwenOmniRealtimeSession(RealtimeSession):
    ...
    def _send_session_update(self):
        config = {
            ...
            "turn_detection": {
                ...
                "silence_duration_ms": int(__import__("os").getenv("VAD_SILENCE_MS", "300")), "create_response": True, "interrupt_response": True,
            },
        }
    ...
    async def _handle_speech_stopped(self):
        ...
        self._pending_auto_reply_timer = self._loop.call_later(
            0.45, self._auto_generate_reply
        )

    def _auto_generate_reply(self):
        self._pending_auto_reply_timer = None
        # 这是 server VAD 的自动回复兜底，不能调 generate_reply。后者会创建
        # pending future，令 LiveKit 把生成标成“手动回复”并等待不存在的播放任务。
        if self._current_gen is None and self._session_ready:
            logger.info("服务端未自动回复，补发 server-initiated response.create")
            self._send({
                "event_id": self._eid(),
