...
3. 鼎信 8T 后台配置 SIP Server 为服务器公网 IP，账号/密码匹配服务器 `pjsip.conf`。
4. 后端 `.env` 里 `ASTERISK_HOST=127.0.0.1`，因为后端和 Asterisk 在同一台服务器。
5. 前端预检通过 AMI、Trunk、AudioSocket 后，再做单号试拨。
