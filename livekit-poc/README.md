...
## 状态声明（2026-07-08 更新：核心链路已真机验证）

当天在本地环境（Mac + UC100 网关 + 真实手机）完成验证：

- **上行**：电话 → 网关 → LiveKit → Qwen，客户语音逐字识别 ✅
- **下行**：Qwen → LiveKit 房间，自测录到 AI 语音 4.7 秒 ✅
- **开场白**：接通触发，正常生成 ✅；**打断**：客户插话时 AI 正确停止 ✅
- **转向延迟**：判停 800ms 时实测 ~1.3s；已默认调为 400ms，目标 ~0.9s
- 原"已知风险第 1 条"（DashScope 协议兼容）已兑现为 4 个真实 bug 并全部修复，
  详见 `integration/WORKER_MIGRATION.md`。`agent/qwen_omni_realtime.py` 即验证版适配器，
  **不要换回 openai 插件直连 DashScope**。

未验证部分：8 口鼎信网关的 trunk 对接（本地用的是 UC100 单卡）、服务器规模部署、
长通话稳定性。测试时注意 SIM 风控：同号短呼十来次就会被运营商停机。
